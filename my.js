...
const CsempeAdagoló = {
    ...
    calculate: function() {
        ...
        this.stats.forEach((stats, tipus) => {
            ...
            stats.leftoverRatio = Math.floor(100 * stats.leftoverCm / (stats.count * csempeszélesség));
...
const csemperend = [
    { height: 14, tipus: KekHullam },
    { height: 20, tipus: KekHullam },
    { height: 20, tipus: KekHullam },
    { height: 20, tipus: KekHullam },
    { height: 5, tipus: SzurkeCsik },
...
function csempésFal(width, height, vspan) {
...
scene.add(csempésFal(200, 207, 'center')); // tukros
scene.add(csempésFal(198, 207, 'center').rotateY(Math.PI / 2).translateX(-198)); // radiatoros
scene.add(csempésFal(76, 207, 'center').translateX(198 + 8)); // zuhany bal
scene.add(csempésFal(93, 207, 'center').translateX(198 + 8 + 76).rotateY(-Math.PI / 2)) // zuhany hatso
scene.add(csempésFal(85, 207, 'center').translateX(198 + 8 + 76).rotateY(-Math.PI / 2).translateX(93 + 8)) // kadas hatso
scene.add(csempésFal(149 + 33, 207, 'center').rotateY(Math.PI).translateX(-280).translateZ(-186)) // kadas jobb
